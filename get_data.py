...
class Msw:
    ...
    def swell_height_and_period(self):  # TODO need to change to avg. min/max breaking height
        ...
        for i in range(sampler_num):
            ...
            if (avg_height >= self.swell_high and avg_period >= self.swell_period) or avg_period > 8:
                ...
                if self.sunrise <= date.hour <= self.sunset: #checks if not night time
                    if datetime.now().date() == date.date(): #checks if the day already passed
                        if datetime.now().hour <= date.hour:  #checks if the hour already passed
    ...
    def check_wind(self):
        ...
        relavent_days_tel = self.df_tel_baroch.iloc[mask]
        no_wind_days = []
